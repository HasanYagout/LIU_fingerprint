...
// 3) Pick the required threshold based on which cutoff we've crossed:
    ...
    public static function getStatus(int $studentCode, int $semesterId, ?Carbon $dateForCheck = null): array
    {
        $today = ($dateForCheck ?: Carbon::now())->startOfDay();
        $student  = Student::where('student_id', $studentCode)->firstOrFail();
        $semester = Semester::findOrFail($semesterId);
        ...
        $midtermWindowStart = Carbon::parse($semester->midterm_date)->copy()->subWeeks(2);
        $finalWindowStart   = Carbon::parse($semester->end_date)->copy()->subWeeks(2);
        ...
        // Fetch paid percentage from pivot
        $pivot = $student
            ->semesters()
            ->wherePivot('semester_id', $semester->id)
            ->first()?->pivot;
        ...
        return [
            ...
            'required'      => $required,
            'status'         => $color,
            'currentPeriod' => $currentPeriod, // start, midterm, or final
        ];
    }
